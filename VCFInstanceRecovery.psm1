...
Function New-SDDCManagerOvaDeployment
{
    ...
    $progress = Get-Job | Receive-Job
    If ($progress) {LogMessage -type INFO -message $progress}
    Sleep 30
    Do
    {
        ...
        $progress = Get-Job | Receive-Job
        If ($progress) {LogMessage -type INFO -message $progress}
        If ($jobStatus -eq "Running") {Sleep 30}
...
Function Move-ClusterHostNetworkingTovSS 
{
 ...
 #>
       ...
       <#  LogMessage -type INFO -message "[$vmhost] Exiting Maintenance Mode" 
