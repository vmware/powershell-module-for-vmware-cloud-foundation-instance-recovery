...
Function Confirm-VCFInstanceRecoveryPreReqs
{
    ...
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] 7Zip4PowerShell Module Missing. Please install"
    }
    else 
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] 7Zip4PowerShell Module found"
    }
    ...
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] Posh-SSH Module Missing. Please install"
    } 
    else 
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] Posh-SSH Module found"
    }
    
    $isPowerCLIInstalled = Get-InstalledModule -name "VMware.PowerCLI" -ErrorAction SilentlyContinue
    If (!$isPowerCLIInstalled)
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] PowerCLI Module Missing. Please install"
    }
    else
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] PowerCLI Module found"
    }
    ...
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] PowerVCF Module Missing. Please install"
    } 
    else
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] PowerVCF Module found"
    }
    ...
    If (!($installedSoftware -match "OpenSSL"))
    {
        ...
        If ($openSSLUrl)
        {
            LogMLogMessage -type INFO -message "[$jumpboxName] OpenSSL missing. Please install. Latest version detected is here: $openSSLUrl"
        }
        else 
        {
            LogMLogMessage -type INFO -message "[$jumpboxName] OpenSSL missing. Please install. Unable to detect latest version on web"
        }
    }
    else
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] OpenSSL Utility found"
    }
    ...
    If ($OpenSSLPath)
    {
        ...
        IF ($testOpenSSExe)
        {
            LogMLogMessage -type INFO -message "[$jumpboxName] openssl.exe found in $OpenSSLPath"
        }
        else 
        {
            LogMLogMessage -type INFO -message "[$jumpboxName] $OpenSSLPath was found in environment path, but no openssl.exe was found in that path"
        }

    }
    else 
    {
        LogMLogMessage -type INFO -message "[$jumpboxName] No folder path that looks like OpenSSL was discovered in the environment path variable. Please double check that the location of OpenSSL is included in the path variable"
    }
}
...
Function New-ExtractDataFromSDDCBackup
{
    ...
    #Decrypt Backup
    LogMLogMessage -type INFO -message "[$jumpboxName] Decrypting Backup"
    ...
    #Extract Backup
    LogMLogMessage -type INFO -message "[$jumpboxName] Extracting Backup"
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Reading Password Vault"
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving NSX Manager Details"
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Host and Domain Mappings"
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Host and vCenter Mappings"
    ...
    #Get Host and vCenter Details
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving vCenter Details"
    ...
    #Get Hosts and Pools
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Host and Network Pool Mappings"
    ...
    #Get Network Pools
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Network Pool Details"
    ...
    #Get VDSs
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving vDS Details"
    ...
    #Get Cluster and VDS
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Cluster and vDS Mappings"
    ...
    #Get Clusters
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Cluster Details"
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Cluster and vCenter Mappings"
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Cluster and Domain Mappings"
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Network Pools and Network Mappings"
    ...
    #Get Networks
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Network Details"
    ...
    #Get License Models
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving Licensing Models"
    ...
    #Get License Keys
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving License Keys"
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Assembling Workload Domain Data"
    #GetDomainDetails
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Retrieving SDDC Manager Detail"
    #GetDomainDetails
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Creating extracted-sddc-data.json"
    ...
    #Cleanup
    LogMLogMessage -type INFO -message "[$jumpboxName] Cleaning up extracted files"
...
Function New-NSXManagerOvaDeployment
{
    ...
    LogMLogMessage -type INFO -message "[$nsxManagerVMName] Deploying NSX Manager OVA"
...
Function New-vCenterOvaDeployment
{
    ...
    LogMLogMessage -type INFO -message "[$restoredvCenterVMName] Deploying vCenter OVA"
...
Function New-SDDCManagerOvaDeployment
{
    ...
    LogMLogMessage -type INFO -message "[$sddcManagerVMName] Deploying SDDC Manager OVA"
...
Function New-UploadAndModifySDDCManagerBackup
{
    ...
    $jumpboxName = hostname
    LogMLogMessage -type INFO -message "[$jumpboxName] Reading Extracted Data"
    $extractedDataFilePath = (Resolve-Path -Path $extractedSDDCDataFile).path
    $extractedSddcData = Get-Content $extractedDataFilePath | ConvertFrom-JSON
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Establishing Connection to SDDC Manager Appliance"
    ...
    #Perform KeyScan
    LogMLogMessage -type INFO -message "[$sddcManagerFQDN] Performing Keyscan on SDDC Manager Appliance"
    ...
    If ($newNistKey) { LogMLogMessage -type INFO -message "[$sddcManagerFQDN] New ecdsa-sha2-nistp256 key for $mgmtVcenterFqdn retrieved" }
    $newRSAKey = '"' + (($result | Where-Object {$_ -like "*ssh-rsa*"}).split("ssh-rsa "))[1] + '"'
    If ($newRSAKey) { LogMLogMessage -type INFO -message "[$sddcManagerFQDN] New ssh-rsa key for $mgmtVcenterFqdn retrieved" }

    #Upload Backup
    $vCenterConnection = Connect-VIServer -server $tempvCenterFqdn -user $tempvCenterAdmin -password $tempvCenterAdminPassword
    LogMLogMessage -type INFO -message "[$jumpboxName] Uploading Backup File to SDDC Manager Appliance"
    ...
    LogMLogMessage -type INFO -message "[$sddcManagerFQDN] Decrypting Backup on SDDC Manager Appliance"
    ...
    #Modfiy JSON file  
    #Existing Nist Key
    LogMLogMessage -type INFO -message "[$sddcManagerFQDN] Parsing Backup on SDDC Manager Appliance for original ecdsa-sha2-nistp256 key for $mgmtVcenterFqdn"
    ...
    #Existing rsa Key
    LogMLogMessage -type INFO -message "[$sddcManagerFQDN] Parsing Backup on SDDC Manager Appliance for original ssh-rsa key for $mgmtVcenterFqdn"
    ...
    #Sed File
    LogMLogMessage -type INFO -message "[$sddcManagerFQDN] Replacing ecdsa-sha2-nistp256 and ssh-rsa keys and re-encrypting the SDDC Manager Backup"
...
Function New-ReconstructedPartialBringupJsonSpec
{
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Saving partial bringup JSON spec: $(($extractedSddcData.workloadDomains | Where-Object {$_.domainType -eq "MANAGEMENT"}).domainName + "-partial-bringup-spec.json")"
...
Function Invoke-SDDCManagerRestore
{
    ...
    LogMLogMessage -type INFO -message "[$jumpboxName] Establishing Connection to $extractedSddcManagerFqdn"
    $SecurePassword = ConvertTo-SecureString -String $vcfUserPassword -AsPlainText -Force
    $mycreds = New-Object System.Management.Automation.PSCredential ('vcf', $SecurePassword)
    $inmem = New-SSHMemoryKnownHost
    ...
    LogMLogMessage -type INFO -message "[$extractedSddcManagerFqdn] Configuring Restore Process"
    ...
    #Execute Restore
    LogMLogMessage -type INFO -message "[$extractedSddcManagerFqdn] Performing Restore"
    ...
    If ($token)
    {
        ...
        LogMLogMessage -type WAIT -message "[$extractedSddcManagerFqdn] Waiting for Operations Manager Service to be Up"
        $Counter = 0
        ...
        If ($restoreID)
        {
            ...
            LogMLogMessage -type INFO -message "[$extractedSddcManagerFqdn] Monitoring Restore Task $restoreID progress (polling every 60 seconds)"
            Do
            {
                Sleep 60
                ...
                LogMLogMessage -type INFO -message "[$extractedSddcManagerFqdn] Restore Status: $restoreProgress"
            } While ($restoreProgress -in "IN PROGRESS")    
        }
        else 
        {
            LogMLogMessage -type ERROR -message "[$extractedSddcManagerFqdn] Restore Task ID not returned"
        }
    }
    else 
    {
        LogMLogMessage -type ERROR -message "[$extractedSddcManagerFqdn] Failed to get SDDC Manager Token"
    }
...
Function Move-ClusterHostsToRestoredVcenter
{
    ...
    Foreach ($esxiHost in $esxiHosts) {
        LogMLogMessage -type INFO -message "[$($esxiHost.name)] Moving to $restoredvCenterFQDN"
...
Function Remove-ClusterHostsFromVds
{
    ...
    $esxiHosts = get-cluster -name $clusterName | get-vmhost
    Foreach ($esxiHost in $esxiHosts) {
        LogMLogMessage -type INFO -message "[$($esxiHost.name)] Removing from $vdsName"
...
Function Move-MgmtVmsToTempPg 
{
    ...
    foreach ($vmToMove in $vmsTomove) {
        LogMLogMessage -type INFO -message "[$($vmToMove.name)] Moving to mgmt_temp"
...
Function Move-ClusterHostNetworkingTovSS 
{
    ...
    foreach ($vmhost in $vmhost_array) {
        <# LogMLogMessage -type INFO -message "[$vmhost] Entering Maintenance Mode" 
 ...
 #>
        ...
        LogMLogMessage -type INFO -message "[$vmhost] Retrieving pNIC info for vmnic1"
        ...
        LogMLogMessage -type INFO -message "[$vmhost] Creating $mgmt_name portrgroup on $vss_name"
        $mgmt_pg = New-VirtualPortGroup -VirtualSwitch $vss -Name $mgmt_name -VLanId $mgmtVlanId

        LogMLogMessage -type INFO -message "[$vmhost] Creating $vmotion_name portrgroup on $vss_name"
        $vmotion_pg = New-VirtualPortGroup -VirtualSwitch $vss -Name $vmotion_name -VLanId $vMotionVlanId

        LogMLogMessage -type INFO -message "[$vmhost] Creating $storage_name Network portrgroup on $vss_name"
        ...
        LogMLogMessage -type INFO -message "[$vmhost] Migrating from $vdsName to $vss_name"
       ...
       <#  LogMLogMessage -type INFO -message "[$vmhost] Exiting Maintenance Mode" 
...
Function Move-ClusterVmnicTovSwitch
{
    ...
    $esxiHosts = get-cluster -name $clusterName | get-vmhost
    Foreach ($esxiHost in $esxiHosts) {
        LogMLogMessage -type INFO -message "[$esxiHost] Migrating `'$vmnic`' from vDS to vSwitch0"
...
Function Set-ClusterHostsvSanIgnoreClusterMemberList
{
    ...
    foreach ($esxiHost in $esxiHosts) {
        ...
        LogMLogMessage -type INFO -message "[$esxiHost] Setting vSAN Ignore Cluster Member to `'$setting`'"
...
Function Move-ClusterVMsToFirstHost
{
    ...
    Foreach ($vm in $vms) {
        if ($vm.vmHost.Name -ne $firstHost) {
            ...
            LogMLogMessage -type INFO -message "[$($vm.name)] Moving to $firstHost"
        }
    }
    Do {
...
Function Resolve-PhysicalHostServiceAccounts 
{
    ...
    {
        LogMLogMessage -type INFO -message "[$sddcManagerFQDN] Failed credential operation detected. Please resolve in SDDC Manager and try again" ; break
    }
    ...
    Foreach ($hostInstance in $clusterHosts) {
        ...
        If (!$accountExists) {
            LogMLogMessage -type INFO -message "[$($hostInstance.name)] VCF Service Account Not Found: Creating"
        ...
        {
            LogMLogMessage -type INFO -message "[$($hostInstance.name)] VCF Service Account Found: Setting Password"
    ...
    Foreach ($hostInstance in $clusterHosts) {
        ...
        LogMLogMessage -type INFO -message "[$($hostInstance.name)] Remediating VCF Service Account Password: " -nonewline
        ...
        LogMLogMessage -type INFO -message "$taskStatus"
    }
}
...
Function Remove-NonResponsiveHosts 
{
    ...
    foreach ($nonResponsiveHost in $nonResponsiveHosts) {
        LogMLogMessage -type INFO -message "[$($nonResponsiveHost.name)] Removing from $clusterName"
...
Function Add-HostsToCluster 
{
    ...
    foreach ($newHost in $newHosts) {
        ...
        if ($newHost -notin $vmHosts) {
            ...
            if ($esxiConnection) {
                LogMLogMessage -type INFO -message "[$newHost] Adding to cluster $clusterName"
            ...
            LogMLogMessage -type INFO -message "[$newHost] Already part of $clusterName. Skipping"
        }
    }
    Disconnect-VIServer -Server $global:DefaultVIServers -Force -Confirm:$false
}
...
Function Remove-StandardSwitch 
{
    ...
    foreach ($vmhost in $vmHosts) {
        LogMLogMessage -type INFO -message "[$vmhost] Removing standard vSwitch" 
...
Function Add-VMKernelsToHost 
{
    ...
    foreach ($vmhost in $vmHosts) { 
        ...
        LogMLogMessage -type INFO -message "[$vmhost] Creating vMotion vMK"
        ...
        LogMLogMessage -type INFO -message "[$vmhost] Setting vMotion Gateway"
        $vmkName = 'vmk1'
        ...
        LogMLogMessage -type INFO -message "[$vmhost] Creating vSAN vMK"
        ...
        LogMLogMessage -type INFO -message "[$vmhost] Setting vSAN Gateway"
        $vmkName = 'vmk2'
        $esx = Get-VMHost -Name $vmHost
        $esxcli = Get-EsxCli -VMHost $esx -V2
        $interface = $esxcli.network.ip.interface.ipv4.get.Invoke(@{interfacename = $vmkName })
    ...
    try {
        If (Test-Path -path $jsonFile) {
            ...
            Foreach ($vmOverRideInstance in $vmOverRideInstances)
            {
                ...
                {
                    LogMLogMessage -type INFO -message "[$($vmOverRideInstance.name)] Restoring VM Overide Settings"
                ...
                If ($vmLocation.name -notlike "vCLS*") {
                    ...
                    If ($vm) {
                        If ($vm.folder -ne $vmLocation.folder) {
                            LogMLogMessage -type INFO -message "[$($vmLocation.name)] Setting VM Folder Location to $($vmLocation.folder)"
                            Move-VM -VM $vm -InventoryLocation $vmLocation.folder -confirm:$false
                        }
                        If ($vm.resourcePool -ne $vmLocation.resourcePool) {
                            LogMLogMessage -type INFO -message "[$($vmLocation.name)] Setting ResourcePool to $($vmLocation.resourcePool)"
        ...
        If (Test-Path -path $jsonFile) {
                ...
                If ($group) {
                    If ($vmDrsGroup.type -eq "VMHostGroup") {
                        Foreach ($member in $vmDrsGroup.members) {
                            LogMLogMessage -type INFO -message "[$member] Adding to VMHostGroup $($vmDrsGroup.name)"
                    ...
                    elseif ($vmDrsGroup.type -eq "VMGroup") {
                        Foreach ($member in $vmDrsGroup.members) {
                            LogMLogMessage -type INFO -message "[$member] Adding to VMGroup $($vmDrsGroup.name)"
                    ...
                    If ($vmDrsGroup.type -eq "VMHostGroup") {
                        LogMLogMessage -type INFO -message "[$($vmDrsGroup.name)] Creating VMHostGroup with Members $($vmDrsGroup.members)"
                        New-DrsClusterGroup -Name $vmDrsGroup.name -VMHost $vmDrsGroup.members -Cluster $clusterName | Out-Null
                    }
                    elseif ($vmDrsGroup.type -eq "VMGroup") {
                        LogMLogMessage -type INFO -message "[$($vmDrsGroup.name)] Creating VMGroup with Members $($vmDrsGroup.members)"
            ...
            Foreach ($vmAffinityRule in $drsRulesAndGroups.vmAffinityRules) {
                If ($vmAffinityRule.members.count -gt 1)
                {
                    ...
                    If ($vmRule) {
                        LogMLogMessage -type INFO -message "[$($vmAffinityRule.name)] Setting VM Rule with Members $($vmAffinityRule.members)"
                        Set-DrsRule -rule $vmRule -VM $vmAffinityRule.members -Enabled $true -confirm:$false | Out-Null
                    }
                    else {
                        LogMLogMessage -type INFO -message "[$($vmAffinityRule.name)] Creating VM Rule with Members $($vmAffinityRule.members)"
            ...
            Foreach ($vmHostAffinityRule in $drsRulesAndGroups.vmHostAffinityRules) {
                ...
                If ($hostRule) {
                    LogMLogMessage -type INFO -message "[$($vmHostAffinityRule.name)] Setting VMHost Rule with VM Group $($vmHostAffinityRule.vmGroupName) and Host Group $($vmHostAffinityRule.hostGroupName)"
                    ...
                    LogMLogMessage -type INFO -message "[$($vmHostAffinityRule.name)] Creating VMHost Rule with VM Group $($vmHostAffinityRule.vmGroupName) and Host Group $($vmHostAffinityRule.hostGroupName)"
            ...
            Foreach ($vmToVmDependencyRule in $drsRulesAndGroups.vmToVmDependencyRules) {
                ...
                If (!$dependencyRule) {
                    LogMLogMessage -type INFO -message "[$($vmToVmDependencyRule.vmGroup)] Creating VM to VM Dependency Rule to depend on $($vmToVmDependencyRule.DependsOnVmGroup) "
                    $cluster = Get-Cluster -Name $clusterName
                    $spec = New-Object VMware.Vim.ClusterConfigSpecEx
...
Function Resolve-PhysicalHostTransportNodes
{
    ...
    $vCenterConnection = Connect-VIServer -server $vCenterFQDN -username $vCenterAdmin -password $vCenterAdminPassword
    LogMLogMessage -type INFO -message "[$clusterName] Getting Hosts"
    ...
    LogMLogMessage -type INFO -message "[$nsxManagerFqdn] Getting Transport Nodes"
    ...
    LogMLogMessage -type INFO -message "[$nsxManagerFqdn] Filtering Transport Nodes to members of cluster $clusterName"
    ...
    Foreach ($hostID in $hostIDs) {
        ...
        LogMLogMessage -type INFO -message "[$nsxManagerFqdn] Resolving NSX Installation on $(($allHostTransportNodes | Where-Object {$_.id -eq $hostID}).display_name) "
...
Function Invoke-NSXEdgeClusterRecovery
{
    ...
    {
        #Get TransportNodes
        LogMLogMessage -type INFO -message "[$nsxManagerFqdn] Looking for Edges to recover in $($edgeLocation.type): $($edgeLocation.name)"
        $headers = VCFIRCreateHeader -username $nsxManagerAdmin -password $nsxManagerAdminPassword
        $uri = "https://$nsxManagerFqdn/api/v1/transport-nodes/"
        $transportNodeContents = (Invoke-WebRequest -Method GET -URI $uri -ContentType application/json -headers $headers).content | ConvertFrom-Json
        ...
        {
            LogMLogMessage -type INFO -message "[$nsxManagerFqdn] Found Edges to recover: $($allEdgeTransportNodes.display_name -join(","))"
        }
        else 
        {
            LogMLogMessage -type INFO -message "[$nsxManagerFqdn] No Edges found needing recovery"
        }
        ...
        Foreach ($edge in $allEdgeTransportNodes)
        {
            ...
            If (!$edgeVmPresent)
            {
                ...
                LogMLogMessage -type INFO -message "[$($edge.display_name)] Getting Placement References"
                ...
                #Create Dummy VM
                LogMLogMessage -type INFO -message "[$($edge.display_name)] Preparing to Update Placement References"
                ...
                LogMLogMessage -type INFO -message "[$($edge.display_name)] Updating Placement References"
                ...
                #Redeploy Edge
                LogMLogMessage -type INFO -message "[$($edge.display_name)] Getting Edge State"
                ...
                {
                    LogMLogMessage -type INFO -message "[$($edge.display_name)] State is $($edgeState.node_deployment_state.state)"
                    If ($edgeState.node_deployment_state.state -in "MPA_DISCONNECTED","VM_PLACEMENT_REFRESH_FAILED","NODE_READY")
                    {
                        LogMLogMessage -type INFO -message "[$($edge.display_name)] Redeploying Edge"
                        $uri = "https://$nsxManagerFqdn/api/v1/transport-nodes/$($edge.node_id)"
                    ...
                    {   
                        LogMLogMessage -type INFO -message "[$($edge.display_name)] Not in a suitable state for redeployment. Please review and retry"
                    }
                }
            }
        }
