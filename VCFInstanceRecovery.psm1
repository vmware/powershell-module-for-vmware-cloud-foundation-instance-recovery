...
Function Confirm-VCFInstanceRecoveryPreReqs
{
    ...
    {
        LogMessage -message "[$jumpboxName] 7Zip4PowerShell Module Missing. Please install"
    }
    else 
    {
        LogMessage -message "[$jumpboxName] 7Zip4PowerShell Module found"
    }
    ...
    {
        LogMessage -message "[$jumpboxName] Posh-SSH Module Missing. Please install"
    } 
    else 
    {
        LogMessage -message "[$jumpboxName] Posh-SSH Module found"
    }
    
    $isPowerCLIInstalled = Get-InstalledModule -name "VMware.PowerCLI" -ErrorAction SilentlyContinue
    If (!$isPowerCLIInstalled)
    {
        LogMessage -message "[$jumpboxName] PowerCLI Module Missing. Please install"
    }
    else
    {
        LogMessage -message "[$jumpboxName] PowerCLI Module found"
    }
    ...
    {
        LogMessage -message "[$jumpboxName] PowerVCF Module Missing. Please install"
    } 
    else
    {
        LogMessage -message "[$jumpboxName] PowerVCF Module found"
    }
    ...
    If (!($installedSoftware -match "OpenSSL"))
    {
        ...
        If ($openSSLUrl)
        {
            LogMessage -message "[$jumpboxName] OpenSSL missing. Please install. Latest version detected is here: $openSSLUrl"
        }
        else 
        {
            LogMessage -message "[$jumpboxName] OpenSSL missing. Please install. Unable to detect latest version on web"
        }
    }
    else
    {
        LogMessage -message "[$jumpboxName] OpenSSL Utility found"
    }
    ...
    If ($OpenSSLPath)
    {
        ...
        IF ($testOpenSSExe)
        {
            LogMessage -message "[$jumpboxName] openssl.exe found in $OpenSSLPath"
        }
        else 
        {
            LogMessage -message "[$jumpboxName] $OpenSSLPath was found in environment path, but no openssl.exe was found in that path"
        }

    }
    else 
    {
        LogMessage -message "[$jumpboxName] No folder path that looks like OpenSSL was discovered in the environment path variable. Please double check that the location of OpenSSL is included in the path variable"
    }
}
...
Function New-ExtractDataFromSDDCBackup
{
    ...
    #Decrypt Backup
    LogMessage -message "[$jumpboxName] Decrypting Backup"
    ...
    #Extract Backup
    LogMessage -message "[$jumpboxName] Extracting Backup"
    ...
    LogMessage -message "[$jumpboxName] Reading Password Vault"
    ...
    LogMessage -message "[$jumpboxName] Retrieving NSX Manager Details"
    ...
    LogMessage -message "[$jumpboxName] Retrieving Host and Domain Mappings"
    ...
    LogMessage -message "[$jumpboxName] Retrieving Host and vCenter Mappings"
    ...
    #Get Host and vCenter Details
    LogMessage -message "[$jumpboxName] Retrieving vCenter Details"
    ...
    #Get Hosts and Pools
    LogMessage -message "[$jumpboxName] Retrieving Host and Network Pool Mappings"
    ...
    #Get Network Pools
    LogMessage -message "[$jumpboxName] Retrieving Network Pool Details"
    ...
    #Get VDSs
    LogMessage -message "[$jumpboxName] Retrieving vDS Details"
    ...
    #Get Cluster and VDS
    LogMessage -message "[$jumpboxName] Retrieving Cluster and vDS Mappings"
    ...
    #Get Clusters
    LogMessage -message "[$jumpboxName] Retrieving Cluster Details"
    ...
    LogMessage -message "[$jumpboxName] Retrieving Cluster and vCenter Mappings"
    ...
    LogMessage -message "[$jumpboxName] Retrieving Cluster and Domain Mappings"
    ...
    LogMessage -message "[$jumpboxName] Retrieving Network Pools and Network Mappings"
    ...
    #Get Networks
    LogMessage -message "[$jumpboxName] Retrieving Network Details"
    ...
    #Get License Models
    LogMessage -message "[$jumpboxName] Retrieving Licensing Models"
    ...
    #Get License Keys
    LogMessage -message "[$jumpboxName] Retrieving License Keys"
    ...
    LogMessage -message "[$jumpboxName] Assembling Workload Domain Data"
    #GetDomainDetails
    ...
    LogMessage -message "[$jumpboxName] Retrieving SDDC Manager Detail"
    #GetDomainDetails
    ...
    LogMessage -message "[$jumpboxName] Creating extracted-sddc-data.json"
    ...
    #Cleanup
    LogMessage -message "[$jumpboxName] Cleaning up extracted files"
...
Function New-NSXManagerOvaDeployment
{
    ...
    LogMessage -message "[$nsxManagerVMName] Deploying NSX Manager OVA"
...
Function New-vCenterOvaDeployment
{
    ...
    LogMessage -message "[$restoredvCenterVMName] Deploying vCenter OVA"
...
Function New-SDDCManagerOvaDeployment
{
    ...
    LogMessage -message "[$sddcManagerVMName] Deploying SDDC Manager OVA"
...
Function New-UploadAndModifySDDCManagerBackup
{
    ...
    $jumpboxName = hostname
    LogMessage -message "[$jumpboxName] Reading Extracted Data"
    $extractedDataFilePath = (Resolve-Path -Path $extractedSDDCDataFile).path
    $extractedSddcData = Get-Content $extractedDataFilePath | ConvertFrom-JSON
    ...
    LogMessage -message "[$jumpboxName] Establishing Connection to SDDC Manager Appliance"
    ...
    #Perform KeyScan
    LogMessage -message "[$sddcManagerFQDN] Performing Keyscan on SDDC Manager Appliance"
    ...
    If ($newNistKey) { LogMessage -message "[$sddcManagerFQDN] New ecdsa-sha2-nistp256 key for $mgmtVcenterFqdn retrieved" }
    $newRSAKey = '"' + (($result | Where-Object {$_ -like "*ssh-rsa*"}).split("ssh-rsa "))[1] + '"'
    If ($newRSAKey) { LogMessage -message "[$sddcManagerFQDN] New ssh-rsa key for $mgmtVcenterFqdn retrieved" }

    #Upload Backup
    $vCenterConnection = Connect-VIServer -server $tempvCenterFqdn -user $tempvCenterAdmin -password $tempvCenterAdminPassword
    LogMessage -message "[$jumpboxName] Uploading Backup File to SDDC Manager Appliance"
    ...
    LogMessage -message "[$sddcManagerFQDN] Decrypting Backup on SDDC Manager Appliance"
    ...
    #Modfiy JSON file  
    #Existing Nist Key
    LogMessage -message "[$sddcManagerFQDN] Parsing Backup on SDDC Manager Appliance for original ecdsa-sha2-nistp256 key for $mgmtVcenterFqdn"
    ...
    #Existing rsa Key
    LogMessage -message "[$sddcManagerFQDN] Parsing Backup on SDDC Manager Appliance for original ssh-rsa key for $mgmtVcenterFqdn"
    ...
    #Sed File
    LogMessage -message "[$sddcManagerFQDN] Replacing ecdsa-sha2-nistp256 and ssh-rsa keys and re-encrypting the SDDC Manager Backup"
...
Function New-ReconstructedPartialBringupJsonSpec
{
    ...
    LogMessage -message "[$jumpboxName] Saving partial bringup JSON spec: $(($extractedSddcData.workloadDomains | Where-Object {$_.domainType -eq "MANAGEMENT"}).domainName + "-partial-bringup-spec.json")"
...
Function Invoke-SDDCManagerRestore
{
    ...
    LogMessage -message "[$jumpboxName] Establishing Connection to $extractedSddcManagerFqdn"
    $SecurePassword = ConvertTo-SecureString -String $vcfUserPassword -AsPlainText -Force
    $mycreds = New-Object System.Management.Automation.PSCredential ('vcf', $SecurePassword)
    $inmem = New-SSHMemoryKnownHost
    ...
    LogMessage -message "[$extractedSddcManagerFqdn] Configuring Restore Process"
    ...
    #Execute Restore
    LogMessage -message "[$extractedSddcManagerFqdn] Performing Restore"
    ...
    If ($token)
    {
        ...
        LogMessage -message "[$extractedSddcManagerFqdn] Waiting for Operations Manager Service to be Up"
        $Counter = 0
        ...
        If ($restoreID)
        {
            ...
            LogMessage -message "[$extractedSddcManagerFqdn] Monitoring Restore Task $restoreID progress (polling every 60 seconds)"
            Do
            {
                Sleep 60
                ...
                LogMessage -message "[$extractedSddcManagerFqdn] Restore Status: $restoreProgress"
            } While ($restoreProgress -in "IN PROGRESS")    
        }
        else 
        {
            LogMessage -message "[$extractedSddcManagerFqdn] Restore Task ID not returned"
        }
    }
    else 
    {
        LogMessage -message "[$extractedSddcManagerFqdn] Failed to get SDDC Manager Token"
    }
...
Function Move-ClusterHostsToRestoredVcenter
{
    ...
    Foreach ($esxiHost in $esxiHosts) {
        LogMessage -message "[$($esxiHost.name)] Moving to $restoredvCenterFQDN"
...
Function Remove-ClusterHostsFromVds
{
    ...
    $esxiHosts = get-cluster -name $clusterName | get-vmhost
    Foreach ($esxiHost in $esxiHosts) {
        LogMessage -message "[$($esxiHost.name)] Removing from $vdsName"
...
Function Move-MgmtVmsToTempPg 
{
    ...
    foreach ($vmToMove in $vmsTomove) {
        LogMessage -message "[$($vmToMove.name)] Moving to mgmt_temp"
...
Function Move-ClusterHostNetworkingTovSS 
{
    ...
    foreach ($vmhost in $vmhost_array) {
        <# LogMessage -message "[$vmhost] Entering Maintenance Mode" 
 ...
 #>
        ...
        LogMessage -message "[$vmhost] Retrieving pNIC info for vmnic1"
        ...
        LogMessage -message "[$vmhost] Creating $mgmt_name portrgroup on $vss_name"
        $mgmt_pg = New-VirtualPortGroup -VirtualSwitch $vss -Name $mgmt_name -VLanId $mgmtVlanId

        LogMessage -message "[$vmhost] Creating $vmotion_name portrgroup on $vss_name"
        $vmotion_pg = New-VirtualPortGroup -VirtualSwitch $vss -Name $vmotion_name -VLanId $vMotionVlanId

        LogMessage -message "[$vmhost] Creating $storage_name Network portrgroup on $vss_name"
        ...
        LogMessage -message "[$vmhost] Migrating from $vdsName to $vss_name"
       ...
       <#  LogMessage -message "[$vmhost] Exiting Maintenance Mode" 
...
Function Move-ClusterVmnicTovSwitch
{
    ...
    $esxiHosts = get-cluster -name $clusterName | get-vmhost
    Foreach ($esxiHost in $esxiHosts) {
        LogMessage -message "[$esxiHost] Migrating `'$vmnic`' from vDS to vSwitch0"
...
Function Set-ClusterHostsvSanIgnoreClusterMemberList
{
    ...
    foreach ($esxiHost in $esxiHosts) {
        ...
        LogMessage -message "[$esxiHost] Setting vSAN Ignore Cluster Member to `'$setting`'"
...
Function Move-ClusterVMsToFirstHost
{
    ...
    Foreach ($vm in $vms) {
        if ($vm.vmHost.Name -ne $firstHost) {
            ...
            LogMessage -message "[$($vm.name)] Moving to $firstHost"
        }
    }
    Do {
...
Function Resolve-PhysicalHostServiceAccounts 
{
    ...
    {
        LogMessage -message "[$sddcManagerFQDN] Failed credential operation detected. Please resolve in SDDC Manager and try again" ; break
    }
    ...
    Foreach ($hostInstance in $clusterHosts) {
        ...
        If (!$accountExists) {
            LogMessage -message "[$($hostInstance.name)] VCF Service Account Not Found: Creating"
        ...
        {
            LogMessage -message "[$($hostInstance.name)] VCF Service Account Found: Setting Password"
    ...
    Foreach ($hostInstance in $clusterHosts) {
        ...
        LogMessage -message "[$($hostInstance.name)] Remediating VCF Service Account Password: " -nonewline
...
Function Remove-NonResponsiveHosts 
{
    ...
    foreach ($nonResponsiveHost in $nonResponsiveHosts) {
        LogMessage -message "[$($nonResponsiveHost.name)] Removing from $clusterName"
...
Function Add-HostsToCluster 
{
    ...
    foreach ($newHost in $newHosts) {
        ...
        if ($newHost -notin $vmHosts) {
            ...
            if ($esxiConnection) {
                LogMessage -message "[$newHost] Adding to cluster $clusterName"
            ...
            LogMessage -message "[$newHost] Already part of $clusterName. Skipping"
        }
    }
    Disconnect-VIServer -Server $global:DefaultVIServers -Force -Confirm:$false
}
...
Function Remove-StandardSwitch 
{
    ...
    foreach ($vmhost in $vmHosts) {
        LogMessage -message "[$vmhost] Removing standard vSwitch" 
...
Function Add-VMKernelsToHost 
{
    ...
    foreach ($vmhost in $vmHosts) { 
        ...
        LogMessage -message "[$vmhost] Creating vMotion vMK"
        ...
        LogMessage -message "[$vmhost] Setting vMotion Gateway"
        $vmkName = 'vmk1'
        ...
        LogMessage -message "[$vmhost] Creating vSAN vMK"
        ...
        LogMessage -message "[$vmhost] Setting vSAN Gateway"
        $vmkName = 'vmk2'
        $esx = Get-VMHost -Name $vmHost
        $esxcli = Get-EsxCli -VMHost $esx -V2
        $interface = $esxcli.network.ip.interface.ipv4.get.Invoke(@{interfacename = $vmkName })
    ...
    try {
        If (Test-Path -path $jsonFile) {
            ...
            Foreach ($vmOverRideInstance in $vmOverRideInstances)
            {
                ...
                {
                    LogMessage -message "[$($vmOverRideInstance.name)] Restoring VM Overide Settings"
                ...
                If ($vmLocation.name -notlike "vCLS*") {
                    ...
                    If ($vm) {
                        If ($vm.folder -ne $vmLocation.folder) {
                            LogMessage -message "[$($vmLocation.name)] Setting VM Folder Location to $($vmLocation.folder)"
                            Move-VM -VM $vm -InventoryLocation $vmLocation.folder -confirm:$false
                        }
                        If ($vm.resourcePool -ne $vmLocation.resourcePool) {
                            LogMessage -message "[$($vmLocation.name)] Setting ResourcePool to $($vmLocation.resourcePool)"
        ...
        If (Test-Path -path $jsonFile) {
                ...
                If ($group) {
                    If ($vmDrsGroup.type -eq "VMHostGroup") {
                        Foreach ($member in $vmDrsGroup.members) {
                            LogMessage -message "[$member] Adding to VMHostGroup $($vmDrsGroup.name)"
                    ...
                    elseif ($vmDrsGroup.type -eq "VMGroup") {
                        Foreach ($member in $vmDrsGroup.members) {
                            LogMessage -message "[$member] Adding to VMGroup $($vmDrsGroup.name)"
                    ...
                    If ($vmDrsGroup.type -eq "VMHostGroup") {
                        LogMessage -message "[$($vmDrsGroup.name)] Creating VMHostGroup with Members $($vmDrsGroup.members)"
                        New-DrsClusterGroup -Name $vmDrsGroup.name -VMHost $vmDrsGroup.members -Cluster $clusterName | Out-Null
                    }
                    elseif ($vmDrsGroup.type -eq "VMGroup") {
                        LogMessage -message "[$($vmDrsGroup.name)] Creating VMGroup with Members $($vmDrsGroup.members)"
            ...
            Foreach ($vmAffinityRule in $drsRulesAndGroups.vmAffinityRules) {
                If ($vmAffinityRule.members.count -gt 1)
                {
                    ...
                    If ($vmRule) {
                        LogMessage -message "[$($vmAffinityRule.name)] Setting VM Rule with Members $($vmAffinityRule.members)"
                        Set-DrsRule -rule $vmRule -VM $vmAffinityRule.members -Enabled $true -confirm:$false | Out-Null
                    }
                    else {
                        LogMessage -message "[$($vmAffinityRule.name)] Creating VM Rule with Members $($vmAffinityRule.members)"
            ...
            Foreach ($vmHostAffinityRule in $drsRulesAndGroups.vmHostAffinityRules) {
                ...
                If ($hostRule) {
                    LogMessage -message "[$($vmHostAffinityRule.name)] Setting VMHost Rule with VM Group $($vmHostAffinityRule.vmGroupName) and Host Group $($vmHostAffinityRule.hostGroupName)"
                    ...
                    LogMessage -message "[$($vmHostAffinityRule.name)] Creating VMHost Rule with VM Group $($vmHostAffinityRule.vmGroupName) and Host Group $($vmHostAffinityRule.hostGroupName)"
            ...
            Foreach ($vmToVmDependencyRule in $drsRulesAndGroups.vmToVmDependencyRules) {
                ...
                If (!$dependencyRule) {
                    LogMessage -message "[$($vmToVmDependencyRule.vmGroup)] Creating VM to VM Dependency Rule to depend on $($vmToVmDependencyRule.DependsOnVmGroup) "
                    $cluster = Get-Cluster -Name $clusterName
                    $spec = New-Object VMware.Vim.ClusterConfigSpecEx
...
Function Resolve-PhysicalHostTransportNodes
{
    ...
    $vCenterConnection = Connect-VIServer -server $vCenterFQDN -username $vCenterAdmin -password $vCenterAdminPassword
    LogMessage -message "[$clusterName] Getting Hosts"
    ...
    LogMessage -message "[$nsxManagerFqdn] Getting Transport Nodes"
    ...
    LogMessage -message "[$nsxManagerFqdn] Filtering Transport Nodes to members of cluster $clusterName"
    ...
    Foreach ($hostID in $hostIDs) {
        ...
        LogMessage -message "[$nsxManagerFqdn] Resolving NSX Installation on $(($allHostTransportNodes | Where-Object {$_.id -eq $hostID}).display_name) "
...
Function Invoke-NSXEdgeClusterRecovery
{
    ...
    {
        #Get TransportNodes
        LogMessage -message "[$nsxManagerFqdn] Looking for Edges to recover in $($edgeLocation.type): $($edgeLocation.name)"
        $headers = VCFIRCreateHeader -username $nsxManagerAdmin -password $nsxManagerAdminPassword
        $uri = "https://$nsxManagerFqdn/api/v1/transport-nodes/"
        $transportNodeContents = (Invoke-WebRequest -Method GET -URI $uri -ContentType application/json -headers $headers).content | ConvertFrom-Json
        ...
        {
            LogMessage -message "[$nsxManagerFqdn] Found Edges to recover: $($allEdgeTransportNodes.display_name -join(","))"
        }
        else 
        {
            LogMessage -message "[$nsxManagerFqdn] No Edges found needing recovery"
        }
        ...
        Foreach ($edge in $allEdgeTransportNodes)
        {
            ...
            If (!$edgeVmPresent)
            {
                ...
                LogMessage -message "[$($edge.display_name)] Getting Placement References"
                ...
                #Create Dummy VM
                LogMessage -message "[$($edge.display_name)] Preparing to Update Placement References"
                ...
                LogMessage -message "[$($edge.display_name)] Updating Placement References"
                ...
                #Redeploy Edge
                LogMessage -message "[$($edge.display_name)] Getting Edge State"
                ...
                {
                    LogMessage -message "[$($edge.display_name)] State is $($edgeState.node_deployment_state.state)"
                    If ($edgeState.node_deployment_state.state -in "MPA_DISCONNECTED","VM_PLACEMENT_REFRESH_FAILED","NODE_READY")
                    {
                        LogMessage -message "[$($edge.display_name)] Redeploying Edge"
                        $uri = "https://$nsxManagerFqdn/api/v1/transport-nodes/$($edge.node_id)"
                    ...
                    {   
                        LogMessage -message "[$($edge.display_name)] Not in a suitable state for redeployment. Please review and retry"
                    }
                }
            }
        }
