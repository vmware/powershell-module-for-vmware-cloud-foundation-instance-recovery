...
Function New-ExtractDataFromSDDCBackup {
    ...
    $vCenters = @()
    Do {
        $lineContent = $psqlContent | Select-Object -Index $vCenterLineIndex
        If ($lineContent -ne '\.') {
...
Function New-VVFBasedPartialBringupJsonSpec {
    ...
    #MANAGEMENT network
    [IPAddress] $ip = (($defaultManagementCluster | Where-Object { $_.isDefault -eq 't' }).hosts[0].networks | Where-Object { $_.type -eq 'MANAGEMENT' }).mask
    $octets = $ip.IPAddressToString.Split('.')
...
Function New-VVFBasedPartialBringupValidation {
    ...
    $response.validationChecks | Select-Object Description, ResultStatus | Format-Table -AutoSize -Wrap | Out-String -Stream | ForEach-Object { " $_" }
    $StopWatch.Stop()
...
Function New-VVFBasedPartialBringup {
    ...
    $response.sddcSubTasks | Select-Object name, status | Format-Table -AutoSize -Wrap | Out-String -Stream | ForEach-Object { " $_" }
    $StopWatch.Stop()
