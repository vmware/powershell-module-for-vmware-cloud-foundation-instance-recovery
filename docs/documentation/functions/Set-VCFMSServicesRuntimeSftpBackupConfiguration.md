...
Applies SFTP backup settings to a VCFMS Services Runtime component.
...
The `Set-VcfmsSftpBackupSettings` cmdlet applies SFTP backup configuration to the specified VCFMS component via `POST /api/v1/components/{ComponentId}?action=apply`. If `-SftpFingerprint` is not supplied, the SSH host key fingerprint is automatically retrieved from the SFTP server using `ssh-keyscan`. The request body matches the documented `spec.configuration.backups.storage.sftp` structure and includes an empty `options` object.
...
Optional. The SHA-256 fingerprint of the SFTP server's SSH host key (e.g. `SHA256:abc123...`). If omitted, it is retrieved automatically via `ssh-keyscan`.
