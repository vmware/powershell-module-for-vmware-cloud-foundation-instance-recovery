...
NAMESPACE="${COMPONENT_NAMESPACE[$COMPONENT_TYPE]}"
...
if kubectl get pd "${PD_NAME}" -n "${NAMESPACE}" -o yaml > "${BACKUP_DIR}/${PD_NAME}-pd.yaml" 2>/dev/null; then
