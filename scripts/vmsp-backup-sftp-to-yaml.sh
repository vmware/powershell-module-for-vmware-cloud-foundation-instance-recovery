...
sftp_list_names() {
  ...
  sftp_batch "$pass" "cd ${rdir}" 'ls -1' 'bye' >"$tmp" 2>&1 || { cat "$tmp" >&2; rm -f "$tmp"; return 1; }
  # Drop noise lines
...
sftp_download() {
  ...
  local tmp
  tmp=$(mktemp)
  sftp_batch "$pass" "get ${remote_path} ${local_path}" 'bye' >"$tmp" 2>&1 || { cat "$tmp" >&2; rm -f "$tmp"; return 1; }
  rm -f "$tmp"
}
...
main() {
  ...
  if [[ -n "$REMOTE_ARCHIVE" && -f "$REMOTE_ARCHIVE" ]]; then
    ...
    if [[ "$REMOTE_ARCHIVE" == /* ]]; then
      die "--remote-archive should be relative to SFTP login (no leading /)"
    fi
